...
exports.helperchillbot = functions.https.onRequest((request, response) => {
  ...
  // Start a new round of the chillbot by selecting new questions
  const startNewRound = (callback) => {

    firebaseAdmin.database().ref(DATABASE_DRINKS)
      .once('value', (data) => {
        drinkDictionary = data.val();
        app.data.drinkDictionary = drinkDictionary;
    });
        ...
        if (data && data.val() && data.val()[DATABASE_QUESTIONS]) {
          ...
          if (selectedQuestions) {
            ...
            if (selectedAnswers) {
              ...
              // Session data for the chillbot logic
              ...
              app.data.chillbotLength = chillbotLength;
              app.data.fallbackCount = 0;
              callback(null);
            } else {
